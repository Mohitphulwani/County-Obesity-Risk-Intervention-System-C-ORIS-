...
# --- CONFIGURATION & SETUP ---
MAIN_DATA_FILE = 'StateAndCountyData.csv'
VAR_LIST_FILE = 'VariableList.csv'

st.set_page_config(
    page_title="US Health Disparities & Obesity Analysis", 
    layout="wide", 
...
<style>
    ...
    /* 3. WIDGET LABELS (Fix for Sidebar Inputs) */
    ...
    /* 4. DROPDOWN MENUS (The "Invisible Text" Fix) */
    /* The clickable box */
...
st.title("🏥 Advanced Machine Learning for Understanding & Predicting Health Disparities")
st.subheader("A Comprehensive Analysis of US County Obesity Rates")
...
def generate_demo_data():
    ...
    st.warning("⚠️ CSV files not found. Using Synthetic Demo Data for demonstration.")
    np.random.seed(42)
    n_counties = 500
...
@st.cache_data
def load_and_prepare_data(main_file: str, var_file: str) -> Tuple[pd.DataFrame, str, Dict]:
    st.info("Loading national health and socioeconomic data...")
    try:
        nan_values = ['nan', 'NaN', '00nan', '00NaN', -9999.0, -8888.0]
        df_long = pd.read_csv(main_file, dtype={'FIPS': str}, na_values=nan_values)
        df_vars = pd.read_csv(var_file)
    ...
    st.success(f"✅ Data loaded: {len(df_clean)} counties.")
    return df_clean.reset_index(drop=True), target_col, var_map

# Initialize Session State
if 'df_wide' not in st.session_state:
    df, target, vmap = load_and_prepare_data(MAIN_DATA_FILE, VAR_LIST_FILE)
...
def create_download_pdf(county_name, analysis_data, notes, target_name, target_val, predicted_val):
    ...
    for item in analysis_data:
        # Safe string truncation to avoid table breaking
        name = str(item['name'])
    ...
    # Sanitize notes to avoid latin-1 encoding errors
    safe_notes = (notes if notes else "No specific notes.").encode('latin-1', 'replace').decode('latin-1')
    pdf.multi_cell(0, 6, txt=safe_notes)
    
    # --- THE FIX IS HERE ---
    # We check if the output is a string (Old FPDF) or Bytes (New FPDF)
    pdf_output = pdf.output(dest='S')
    
    if isinstance(pdf_output, str):
        return pdf_output.encode('latin-1') # Old version needs encoding
    else:
        return bytes(pdf_output) # New version is already bytes
# ==============================================================================
